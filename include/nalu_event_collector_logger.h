 ...
 *
 * This class provides logging functionality with different log levels (DEBUG,
 * INFO, WARNING, ERROR), allowing logs to be output to both the console (with
 * optional color formatting) and a log file. The logging behavior can be
 * customized by adjusting the log level, enabling/disabling file logging, and
 * changing the color scheme for each log level.
 */
class NaluEventCollectorLogger {
   public:
     ...
     */
    static void set_log_colors(const std::string& debug_color,
   ...
   private:
     ...
     */
    static std::string get_colored_message(const std::string& message,
