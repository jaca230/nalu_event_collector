 ...
 *
 * This class provides methods to append data to the buffer, pop individual
 * bytes, retrieve all bytes, and wait for the buffer to contain a minimum
 * number of bytes. It also supports overflow notifications and checks if the
...
class NaluUdpDataBuffer {
   public:
     ...
     *
     * This method inserts the provided data into the buffer. If appending the
     * data would exceed the buffer's capacity, an overflow error is thrown, and
     * an overflow callback (if set) is invoked.
     *
     ...
     *
     * This method removes and returns a single byte from the front of the
     * buffer. It returns false if the buffer is empty.
     *
     * @param byte Reference to a byte variable to store the popped byte.
     * @return True if a byte is successfully popped, false if the buffer is
     * empty.
     */
     ...
     *
     * This method copies all the bytes from the buffer and clears the buffer
     * afterward.
     *
    ...
    /**
     * @brief Wait until at least a specified number of bytes are available in
     * the buffer.
     *
     ...
     *
     * This method sets a callback function that will be called if the buffer
     * overflows (i.e., when attempting to append more data than the buffer can
     * hold).
     *
   ...
   private:
    ...
    std::condition_variable
        cv_;  ///< Condition variable for waiting when needed.

    std::function<void()> overflow_callback_;  ///< Callback function to be
