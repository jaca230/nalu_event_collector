 ...
 * @brief This file defines the NaluPacketParser class responsible for parsing
 * Nalu packets from a byte stream.
 *
 * The NaluPacketParser class is used to process raw byte streams, identify
 * valid Nalu packets, and store them in a vector of NaluPacket structures. The
 * class includes functionality to validate packet integrity with start and stop
 * markers, extract relevant data from the byte stream, and handle any leftover
 * or incomplete packets.
 *
 * The class is configurable with several parameters such as packet size, marker
 * values, and timing masks.
 *
 * @note The parsing process can be customized by setting flags for checking
 * packet integrity.
 */
 ...
 *
 * The NaluPacketParser processes byte streams, identifies Nalu packets based on
 * specified start and stop markers, and extracts relevant data into NaluPacket
 * structures. It supports both marker-based integrity checks and raw parsing
...
class NaluPacketParser {
   public:
     ...
     * @param start_marker The start marker used to identify the beginning of a
     * packet.
     ...
     * @param check_packet_integrity Flag to enable or disable packet integrity
     * checks.
     * @param constructed_packet_header The header to be used for constructed
     * packets.
     * @param constructed_packet_footer The footer to be used for constructed
     * packets.
     */
    ...
    /**
     * @brief Constructor that accepts parameters encapsulated in a parameter
     * struct.
     *
     * @param params The NaluPacketParserParams struct containing the
     ...
     *
     * This method splits the byte stream into Nalu packets, validates each
     * packet, and returns a vector of parsed NaluPacket structures.
     *
     * @param byte_stream The raw byte stream to be processed.
     * @return A vector of NaluPacket structures.
     */
    std::vector<NaluPacket> process_stream(
   ...
   private:
     ...
     *
     * This method extracts the relevant data from the byte stream and stores it
     * in a NaluPacket structure.
     *
     * @param packets The vector of NaluPacket structures to store the parsed
     * packet.
     * @param byte_stream The raw byte stream containing the packet data.
     * @param start_index The starting index within the byte stream to begin
     * parsing.
    ...
    /**
     * @brief Checks if a specific marker is found at the given position in the
     * byte stream.
     *
     ...
     *
     * This method ensures that both the start and stop markers are correctly
     * located in the byte stream segment.
     *
     * @param packets The vector of NaluPacket structures to store the parsed
     * packets.
     * @param byte_stream The raw byte stream.
     ...
     *
     * This method processes the byte stream without validating the markers,
     * used for raw data parsing.
     *
     * @param packets The vector of NaluPacket structures to store the parsed
     * packets.
     * @param byte_stream The raw byte stream.
     * @param i The current position within the byte stream.
     * @param error_code The error code associated with packet integrity (not
     * used).
     ...
     *
     * This method ensures that any incomplete packet data from the leftovers
     * buffer is handled properly.
     *
