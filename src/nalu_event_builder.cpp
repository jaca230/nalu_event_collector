...
NaluEventBuilder::NaluEventBuilder(std::vector<int> channels, int windows, 
                                   ...
                                   size_t max_lookback /*= 2*/, size_t event_buffer_max_size /*= 1024*/, 
                                   uint16_t event_header /*= 0xBBBB*/, uint16_t event_trailer /*= 0xEEEE*/)
    : channels(std::move(channels)), windows(windows),
      time_diff_calculator(max_trigger_time, time_threshold),
      event_buffer(event_buffer_max_size, time_diff_calculator, max_lookback, this->channels.size() * windows + 5, event_header, event_trailer) {
