...
public:
    ...
    struct Header {
        ...
        /** @brief Microseconds to wait after the last packet before marking an event as complete (4 bytes). */
     ...
     * @param event_completion_time_us Microseconds to wait after the last packet before marking an event as complete.
     */
    NaluEvent(uint16_t hdr, uint8_t extra_info, uint32_t idx,
              uint32_t ref_time, uint32_t time_thresh, uint32_t clk_freq, uint32_t event_completion_time_us,
