...
NaluEvent::NaluEvent(uint16_t hdr, uint16_t extra_info, uint32_t idx,
    ...
    packets = std::make_unique<NaluPacket[]>(
        max_num_packets);  // Allocate memory for packets using unique_ptr
    // Initialize creation timestamp to current time
    creation_timestamp = std::chrono::steady_clock::now();
}
...
size_t NaluEvent::get_size() const {
    size_t total_size = sizeof(header) + sizeof(info) + sizeof(index) +
                        sizeof(reference_time) + sizeof(packet_size) +
                        sizeof(num_packets) + sizeof(footer);
...
void NaluEvent::serialize_to_buffer(char* buffer) const {
    ...
    std::memcpy(buffer + offset, this,
                offsetof(NaluEvent, packets));  // Up to packets
    offset += offsetof(NaluEvent, packets);

    // Copy packet data (can be large, but it's contiguous memory)
    std::memcpy(buffer + offset, packets.get(),
                num_packets * sizeof(NaluPacket));
...
void NaluEvent::add_packet(const NaluPacket& packet) {
    if (num_packets < max_packets) {
        ...
        NaluEventCollectorLogger::error(
            ...
            ", Current count: " + std::to_string(num_packets));
...
std::chrono::steady_clock::time_point NaluEvent::get_creation_timestamp()
    const {
