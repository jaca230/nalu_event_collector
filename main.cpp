...
int main(int argc, char** argv) {
  ...
  if (run_in_background) {
    ...
    std::this_thread::sleep_for(
        std::chrono::seconds(10));  // Run for 10 seconds (can be adjusted)
    ...
    for (int i = 0; i < 10; ++i) {  // Run for 100 cycles (can be changed)
      ...
      for (size_t i = 0; i < timing_buffer_size; ++i) {
          std::cout << std::hex << std::uppercase << (0xFF &
      static_cast<unsigned int>(timing_buffer[i])) << " "; if ((i + 1) % 16 ==
      0) { std::cout << std::endl;  // Print new line after every 16 bytes for
      readability
          }
      }
      ...
      */
      ...
      "Reference Time: " << event->reference_time << "\n"; std::cout << "Packet
      Size: " << static_cast<int>(event->packet_size) << "\n"; std::cout <<
      "Number of Packets: " << event->num_packets << "\n"; std::cout << "Footer:
      0x" << std::hex << event->footer << std::dec << "\n"; std::cout <<
      "Creation Timestamp: " <<
      event->get_creation_timestamp().time_since_epoch().count() << " ns\n";
          std::cout << "-------------------------------------------\n";
