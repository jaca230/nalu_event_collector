...
NaluEventCollector::NaluEventCollector(const NaluEventCollectorParams& params)
    : receiver(params.udp_receiver_params),  // Unpack parameters for receiver
      ...
      event_builder(params.event_builder_params),  // Unpack parameters for event_builder
      running(false),
      last_event_index(0),  // Initialize to -1, meaning no events have been processed yet
      cycle_count(0),
...
std::vector<NaluEvent*> NaluEventCollector::get_events() {
    ...
    std::vector<NaluEvent*> new_events = event_builder.get_event_buffer().get_events_after_index_inclusive(last_event_index);
...
void NaluEventCollector::printPerformanceStats() {
    ...
    std::cout << "\n\033[1;32mRolling Average (" << cycle_count << "):\033[0m\n";
    std::cout << "+-------------------------+-------------------------+-------------------------+-------------------------+-------------------------+-------------------------+\n";
    std::cout << "| " << std::setw(23) << "Avg Data Rate (MB/s)"
              ...
              << " |\n";
    std::cout << "+-------------------------+-------------------------+-------------------------+-------------------------+-------------------------+-------------------------+\n";
    std::cout << "| " << std::setw(23) << std::fixed << std::setprecision(6) << avg_data_rate
              << " | " << std::setw(23) << avg_parse_time * 1e6
              << " | " << std::setw(23) << avg_event_time * 1e6
              << " | " << std::setw(23) << avg_udp_time * 1e6
              << " | " << std::setw(23) << avg_total_time * 1e6
