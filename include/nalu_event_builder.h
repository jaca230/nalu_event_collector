 ...
 * 
 * The `NaluEventBuilder` class is responsible for collecting events from Nalu packets.
 * It manages the event buffer, handles the safety buffer zone, and provides methods
 * for collecting events and interacting with the internal event buffer.
 */
...
public:
    ...
    /**
     * @brief Constructor that initializes NaluEventBuilder with parameters from NaluEventCollectorParams.
     * 
     * @param params The parameters for the NaluEventBuilder taken from NaluEventCollectorParams.
     */
     ...
     * 
     * This method processes the given packets and adds them to the event buffer, 
     * managing the safety buffer and event index.
     * 
...
private:
    ...
    size_t post_event_safety_buffer_counter_max; ///< The maximum safety buffer size
    size_t post_event_safety_buffer_counter = 0; ///< The current safety buffer counter
    bool in_safety_buffer_zone = false;          ///< Flag indicating whether we're in the safety buffer zone

    uint32_t event_index = 0;           ///< The event index tracker

    NaluEventBuffer event_buffer;       ///< The internal event buffer for managing events
    NaluTimeDifferenceCalculator time_diff_calculator; ///< Time difference calculator

    /**
     * @brief Manages the safety buffer by adjusting the buffer counter and determining when to exit the safety zone.
     */
